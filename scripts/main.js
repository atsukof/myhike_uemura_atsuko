    ...
    db.collection(collection).orderBy("length").get()   //the collection called "hikes"
        .then(allHikes => {
            ...
            allHikes.forEach(doc => { //iterate thru each doc
                ...
                newcard.querySelector('i').id = 'save-' + docID;   // unique id for each hike
...
function updateBookmark(hikeID) {
    currentUser.get().then(userDoc=>{
        ...
        let isBookmarked = bookmarks.includes(hikeID); // check if this hike ID exist in bookmark array
        ...
        if (isBookmarked) {
            ...
            currentUser.update({
                bookmarks: firebase.firestore.FieldValue.arrayUnion(hikeID) // is arrayUnion check if the argument is unique?
            }).then(()=>{
                console.log(hikeID+ " is added to bookmark.")
